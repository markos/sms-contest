// $Id: client_acceptor.cpp,v 2.3 2003/07/22 16:13:24 nmargar Exp $
...
Client_Acceptor::Client_Acceptor (Contest *myContest, concurrency_t concurrency)
...
Client_Acceptor::Client_Acceptor (Contest *myContest, Thread_Pool &thread_pool)
...
/** Registers the Client_Handler Reactor to the Acceptor's reactor,
    while also setting the parentContest object to this->getContest().
*/

int Client_Acceptor::make_svc_handler(Client_Handler *&ch)
{
    ACE_NEW_RETURN(ch,Client_Handler,-1);
    ch->reactor(this->reactor());
    ch->setContest(this->getContest());
