// $Id: client_handler.cpp,v 2.4 2003/08/25 08:32:49 markos Exp $
...
{
  data = new unsigned char[connectionMsgBlock::DATASIZE];
  srv_counter=0;
}
...
{
  while (1)
    if (this->process (data, connectionMsgBlock::DATASIZE) == -1)
      return -1;
...
void Client_Handler::setContest(Contest *mycontest) {
	parentContest = mycontest;
	return;
}

Contest *Client_Handler::getContest() {
