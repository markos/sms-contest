// $Id: client_acceptor.h,v 2.5 2003/08/25 08:30:18 markos Exp $
...
/**
	...
	* \brief	The ACE Acceptor that is used in evalserver
  *
  * The acceptor supports three strategies: single-threaded, thread-per-connection
  * and thread-pool. You can choose between one of these using the concurrency
  * parameter in the constructor.
  * Although the acceptor could operate using a number of I/O methods, we choose
  * to use SOCK_Acceptor objects.
  */
...
public:
  ...
  {
    if (&the_thread_pool_ == &private_thread_pool_)
      return true;
    else
      return false;
  }
  ...
  ///Set parent contest
  void setContest(Contest* curContest) { parentContest = curContest; }

  ///Get parent contest
  Contest* getContest() { return parentContest; }
